---
title: ""
date: 2023-12-06
draft: false
---

{{< rawhtml >}}
<!-- <p style="text-align: center; font-size:24px;"><i>Vision: To make the world a level playing field for everyone </i></p> -->
<div style="display: flex; align-items: flex-start;">
    <div style="flex: 40%; display: flex; flex-direction: column; align-items: center;">
        <img src="/img/Nithish-OD23-DP.jpeg" width=70%>
        <h3>Nithish Krishnabharathi Gnani</h3>
        <h5 style="margin-top: 10px; margin-bottom: 0px;">Technical Associate</h5>
        <h6 style=" margin-top: 10px; margin-bottom: -10px;">Indian Institute of Science (IISc)</h6>
        <p style="text-align: center;">
        Email: nithishgnani [at] iisc.ac.in<br>
        <!-- Email: nkgnani101 [at] gmail.com<br> -->
        <a href="https://scholar.google.com/citations?user=sVE_GcMAAAAJ&hl=en">Google Scholar</a> |
        <a href="https://www.linkedin.com/in/nithish-k-gnani/">LinkedIn</a> |
        <a href="https://github.com/nithishkgnani/">GitHub</a>
        <br><a href="/Nithish-K-Gnani_CV.pdf">CV</a>
        </p>
    </div>
    <div style="flex: 60%;">
        <p style="margin-top: -0px;">
        I'm a researcher at IISc, working with <a href="/projects/">projects</a> on cyber-physical systems (connected robots), tactile internet, IoT data management among other research areas in mechatronic systems. 
        <br>I also design <a href="/project/3d-prints/">3D printed products</a> as a hobby and for work.
        <h5 style="margin-top: -0px; margin-bottom: -10px;">Experience</h5>
        <ul style="padding-left: 20px;" >
            <li>Technical Associate, Centre for Networked Intelligence (CNI), IISc (2022-present)</li>
            <li>Research Staff, Department of Electronic Systems Engineering (DESE), IISc (2019-present)
                <ul>
                    <li>Research Associate, (2021-present)</li>
                    <li>Research Assistant, (2019-2021)</li>
                </ul>
            </li>
            <li>Team Leader, Maintenance at Hindustan Coca-Cola Beverages Private Limited (2017-18)</li>
        </ul>
        <h5 style="margin-top: -0px; margin-bottom: -10px;">Research/Academic Interests</h5>
        <ul style="padding-left: 20px;" >
            <li>Cyber-Physical Systems, Robotics & Control and Mechatronics</li>
            <li>Ultra Reliable Low Latency Communication (uRLLC) - Time Sensitive Networking (TSN)</li>
            <li>Product development</li>
            <li>Indoor localization and sensor identification</li>
            <li>IoT Data Management</li>
        </ul>  
        <!-- <h5 style="margin-top: -0px; margin-bottom: -10px;"><a href="/projects/">Publications 🔗</a></h5> -->
        <!-- <h5 style="margin-top: 30px; margin-bottom: -10px;">Contact</h5>
        <p>
        <b>Email</b><br>
        Work: nithishgnani [at] iisc.ac.in<br>
        Personal: nkgnani101 [at] gmail.com<br>
        <b>Address</b><br>
        ZENLab, DESE, IISc Bangalore, CV Raman Road, Bengaluru, Karnataka, India - 560012 -->
    </div>
</div>
{{< /rawhtml >}}

<!-- {{< rawhtml >}}
<h2>Nithish Krishnabharathi Gnani</h2>
<div style="display: flex; align-items: flex-start;">
    <div style="flex: 30%;">
        <img src="/img/Nithish-OD23-DP.jpeg" width=90%>
        <h5 style="text-align: center; margin-top: 10px; margin-bottom: 0px;">Technical Associate&nbsp;&nbsp;&nbsp;&nbsp;</h5>
        <h6 style="text-align: center; margin-top: 10px; margin-bottom: -10px;">Indian Institute of Science (IISc)&nbsp;&nbsp;&nbsp;&nbsp;</h6>
        <p style="text-align: center;">
        nithishgnani [at] iisc.ac.in<br>
        <a href="https://scholar.google.com/citations?user=sVE_GcMAAAAJ&hl=en">Google Scholar</a> |
        <a href="https://www.linkedin.com/in/nithish-k-gnani/">LinkedIn</a> |
        <a href="https://github.com/nithishkgnani/">GitHub&nbsp;&nbsp;&nbsp;&nbsp;</a>
        <br><a href="/Nithish-K-Gnani_CV.pdf">CV</a>
        </p>
    </div>
    <div style="flex: 70%;">
        <ul style="padding-left: 20px; margin-top: -0px;" >
        <li>Researcher at Indian Institute of Science (IISc).</li>
        <li>Advised by <a href="https://labs.dese.iisc.ac.in/zenlab/people/tv-prabhakar/">Prof. T V Prabhakar</a> and <a href="https://faculty.dese.iisc.ac.in/chandramani/">Prof. Chandramani Singh</a>.</li> 
        <li>Bachelor of Technology (B.Tech) in Mechanical Engineering from National Institute of Technology Karnataka (NITK), India.</li>
        <li>Work: cyber-physical systems (connected robots), tactile internet, IoT data management, and mechatronic systems. <a href="/projects/">Projects page</a>.</li>
        <li>I design <a href="/project/3d-prints/">3D printed products</a> as a hobby and for work.  </li>
        </ul>
        <h5 style="margin-top: -0px; margin-bottom: -10px;">Experience</h5>
        <ul style="padding-left: 20px;" >
            <li>Technical Associate, Centre for Networked Intelligence (CNI), IISc (2022-present)</li>
            <li>Research Staff, Department of Electronic Systems Engineering (DESE), IISc (2019-present)
                <ul>
                    <li>Research Associate, (2021-present)</li>
                    <li>Research Assistant, (2019-2021)</li>
                </ul>
            </li>
            <li>Team Leader, Maintenance at Hindustan Coca-Cola Beverages Private Limited (2017-18)</li>
        </ul>
        <h5 style="margin-top: -0px; margin-bottom: -10px;">Research/Academic Interests</h5>
        <ul style="padding-left: 20px;" >
            <li>Cyber-Physical Systems, Robotics & Control and Mechatronics</li>
            <li>Ultra Reliable Low Latency Communication (uRLLC) - Time Sensitive Networking (IEEE 802.1TSN)</li>
            <li>Indoor localization and sensor identification</li>
            <li>IoT Data Management</li>
        </ul>  
        <h5 style="margin-top: -0px; margin-bottom: -10px;"><a href="/projects/">Publications 🔗</a></h5>
        <h5 style="margin-top: 30px; margin-bottom: -10px;">Contact</h5>
        <p>
        <b>Email</b><br>
        Work: <a href="mailto:nithishgnani@iisc.ac.in">nithishgnani@iisc.ac.in</a><br>
        Personal: <a href="mailto:nkgnani101@gmail.com">nkgnani101@gmail.com</a><br>
        <b>Address</b><br>
        DESE, IISc Bangalore, CV Raman Road, Bengaluru, Karnataka, India - 560012
    </div>
</div>
{{< /rawhtml >}} -->